...
We can see that D.C. ranks towards the bottom of states in terms of population, although not the smallest state. This hasn't always been the case. In fact, there was an almost 50-year stretch in the mid 20th Century where D.C. wasn't even in the bottom 10 states by population.
...
## Taxes
If you ever drive around D.C. and look closely, you'll notice that every license plate has the words "*NO TAXATION WITHOUT REPRESENTATION*". The notion that this country was founded on `r as.integer(format(Sys.Date(), "%Y"))-1776` years ago is one that doesn't hold true **in its own capital**. 
...
tax_cleaned %>% 
  ...
  # Change the theme to classic
    ...
    labs(title = "Taxes Paid by State per Capita (Top 10 and Bottom 5)",
...
Astounding. Compared to every other state, D.C. contributes significantly more to State taxes on a per-capita basis. Per-capita is often the best measure to look at as it shows the average revenue collected per person. Compared to the second-most state, D.C. contributes **`r ((tax_cleaned %>% filter(state == "Dist. of Columbia") %>% select(per_capita) %>% as.integer() ) / (tax_cleaned %>% filter(rank == 2) %>% select(per_capita) %>% as.integer() )) %>% round(1)`x** as much Federal taxes. What drives a differential this large? Well, it's likely driven by the fact that D.C. is, naturally, a much wealthier region of the country per capita compared to all other regions/states (and lobbyists/contractors in the state get paid quite a pretty penny).

Regardless, why does something like the high proportion of taxes paid matter? Well, when Congress was debating the *Alaska Statehood Act* in 1949, Republicans (who are the main opponents of D.C. Statehood) claimed that Alaska's population was too sparse to generate substantial taxes and would ultimately become a welfare state. It's worth noting that Southern Democrats were also opposed to Alaska Statehood for fear that future Alaskan Congresspeople would be pro-Civil Rights. The ultimate passage of the Act was guaranteed by packaging it with Hawaii (a then-Republican territory) Statehood.

## The Power Grab  
There are quite a few arguments about D.C. Statehood being a political ploy by Democrats to gain more control of the U.S. Senate. Those arguments are fair and are likely in the minds of at least some Democratic members of Congress. However, I pose the question: when would the inclusion of 2 likely-Democratic Senators be the least politically motivated? Would it be when Democrats are already in control of all three branches of Congress or when Republicans control the U.S. Senate?

## Is Annexation into MD/VA Possible?
It is! There are strong arguments, not least of which is that Alexandria, VA used to be part of the Federal District (and thus could not vote in Federal Elections), but was [annexed by VA](http://www.compromise-of-1850.org/banning-slave-trade-in-washington-dc/) in order to preserve its ability to maintain slavery. In fact, the retrocession of D.C. to the Federal Government changed significantly over the first ~100 years of this country, as seen in this graphic:

![](images/Dcmapanimated.gif)


Looking closely, one can see that the original portion of land that was ceded to form D.C. is now, exclusively, land given by Maryland.

## Conclusion

Things can change! It wasn't even until 1964 when D.C.'ers were able to vote for U.S. President.