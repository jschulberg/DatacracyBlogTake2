...
author: 'Justin S.'
date: '2020-09-12'
slug: [Women-in-Politics]
featured_image: 'images/power_women.png'
categories:
  - public policy
  - viz
tags:
...
## Predictive Modeling  
In this next section, I'd like to use two predictive models to determine when each chamber of the U.S. Congress will achieve full gender parity. What does that mean in plain English? Can we use data science methods to project when the Senate will have 50 female members and the House 217 (435/2) female members?

The first model I'll use is an ARIMA (Auto Regressive Integrated Moving Average) time forecasting model.
...
Because a linear approach is unrealistic (eventually, the threshold of women in each chamber will eventually exceed 100%) in the long run, let's try to fit something that caps out at 100%. For that reason, I'd like to fit a logistic regression curve, which uses a Sigmoidal function (S-shaped) to make its estimate.
...
# Predict out our future values for 80 years out!
...
# Parity years and numbers
log_house_parity_num <- min(log_house_full$politicians[log_house_full$politicians > 435/2])
log_house_parity_year <- min(log_house_full$year[log_house_full$politicians > 435/2])
log_senate_parity_num <- min(log_senate_full$politicians[log_senate_full$politicians > 100/2])
log_senate_parity_year <- min(log_senate_full$year[log_senate_full$politicians > 100/2])

log_congress_full <- log_house_full %>%
  bind_rows(log_senate_full) %>%
  mutate(parity_year = if_else(chamber == "U.S. House of Representatives",
                                   log_house_parity_year,
                                   log_senate_parity_year),
         parity_number = if_else(chamber == "U.S. House of Representatives",
                               log_house_parity_num,
                               log_senate_parity_num),
         model = "Logistic",
         period = if_else(is.na(period), "historic", period)
         )
...
From this, we can see a much more realistic fit for the model. That is, by using a logistic model, we can safely assume that there is a ceiling to the number of women who will serve in Congress (i.e. 100% of seats being held by women). The logistic models estimate that the **`r if_else(log_senate_parity_year > log_house_parity_year, "House", "Senate")`** will achieve full parity **`r if_else(log_house_parity_year > log_senate_parity_year, log_house_parity_year - log_senate_parity_year, log_senate_parity_year - log_house_parity_year)`** years before the **`r if_else(log_house_parity_year > log_senate_parity_year, "House", "Senate")`**.

For interest purposes, let's just take a look at the ARIMA and Logistic models side-by-side.
...
It seems that the Logistic models, because of their generally steeper ascent to start, lend credence to gender parity in Congress sooner than the ARIMA models estimate.
...
```{r racing_bar_chart_total_by_state, eval=TRUE, include=TRUE}
...
```{r animated_map_total_by_state, eval=TRUE, include=TRUE}
### Map visualization
