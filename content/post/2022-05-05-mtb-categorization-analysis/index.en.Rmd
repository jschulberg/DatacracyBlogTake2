---
title: Mountain Bike Categorization Analysis
author: Justin Schulberg & Mike Czerwinski
date: '2022-05-05'
slug: [mtb-categorization-analysis]
categories:
  - Data
  - Clustering
  ...
  - Bikes
tags: []
featured_image: 'images/bike_cover.png'
...
For this post, I worked with [Mike Czerwinski](https://github.com/mczerwin) to determine whether the specifications of mountain bikes (MTB) are enough to differentiate between the different types of mountain bike categories.

Currently, full suspension mountain bikes come in 5 categories:
...
The data was retrieved manually from each of the mountain bike company's websites. All data and associated code for this analysis is available at the team's corresponding [GitHub repository](https://github.com/mczerwin/MTB-Categorization-Analysis). Let's take a look at the data.
...
# Build our GMM model
mtb_gmm_6 <- GMM(mtb_standard_scaled,
               gaussian_comps = 6,
               dist_mode = 'maha_dist', # Distance metric to use during seeding of initial means clustering
               seed_mode = 'random_subset', # How initial means are seeded prior to EM alg
               km_iter = 10, # Num of iterations of K-Means alg
...
# Run our predictions and convert it to a dataframe
mtb_gmm_6_pred <- predict_GMM(mtb_standard_scaled, 
                            mtb_gmm_6$centroids, 
                            mtb_gmm_6$covariance_matrices, 
                            mtb_gmm_6$weights) 

mtb_gmm_6_pred <- tibble('log_likelihood_c1' = mtb_gmm_6_pred$log_likelihood[, 1], 
                         'log_likelihood_c2' = mtb_gmm_6_pred$log_likelihood[, 2],  
                         'log_likelihood_c3' = mtb_gmm_6_pred$log_likelihood[, 3], 
                         'log_likelihood_c4' = mtb_gmm_6_pred$log_likelihood[, 4], 
                         'log_likelihood_c5' = mtb_gmm_6_pred$log_likelihood[, 5], 
                         'log_likelihood_c6' = mtb_gmm_6_pred$log_likelihood[, 6], 
                         'cluster1_prob' = mtb_gmm_6_pred$cluster_proba[, 1], 
                         'cluster2_prob' = mtb_gmm_6_pred$cluster_proba[, 2], 
                         'cluster3_prob' = mtb_gmm_6_pred$cluster_proba[, 3], 
                         'cluster4_prob' = mtb_gmm_6_pred$cluster_proba[, 4], 
                         'cluster5_prob' = mtb_gmm_6_pred$cluster_proba[, 5], 
                         'cluster6_prob' = mtb_gmm_6_pred$cluster_proba[, 6], 
                         'cluster_labels' = mtb_gmm_6_pred$cluster_labels)
...
mtb_gmm_6_pred %>% 
  ...
  # xtabs(~ ., data = .) %>% 
  table() %>% 
  bind_cols(cluster_labels = c('1', '2', '3', '4', '5', '6')) %>% 
  relocate(cluster_labels, everything()) %>% 
  pander()
```
...
Mapping the Revel Ranger to its first Principal Component we get a value of 1.7 which puts us right around the boundary between Trail and XC, which lines up with the PinkBike editors' labelling of Downcountry in the [video](https://www.youtube.com/watch?v=JYQJbymyG6k) mentioned in the Overview section.
...
Misc.
The extent of this post was written in R. The full blog post can also be found on Medium [here](https://jschulberg.medium.com/mountain-bike-categorization-analysis-61eb1d8bb259). The source code, data, etc. can be found on Mike’s GitHub [here](https://github.com/mczerwin/MTB-Categorization-Analysis). If you want to reach out with thoughts/feedback/ideas for improvement, please either leave a comment on this post or find my contact info on my blog site.
